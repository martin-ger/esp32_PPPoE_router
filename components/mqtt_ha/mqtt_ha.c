...
static void publish_discovery(void)
{
    ...
    /* binary_sensor: PPPoE Session */
    snprintf(topic, sizeof(topic),
        "homeassistant/binary_sensor/%s/pppoe/config", s_device_id);
    snprintf(payload, sizeof(payload),
        "{\"name\":\"PPPoE Session\","
        "\"uniq_id\":\"%s_pppoe\","
        "\"stat_t\":\"" TOPIC_STATE "\","
        "\"val_tpl\":\"{{value_json.pppoe}}\","
        "\"dev_cla\":\"connectivity\","
        "\"pl_on\":\"ON\",\"pl_off\":\"OFF\","
        "\"entity_category\":\"diagnostic\","
        "\"avty_t\":\"" TOPIC_AVAILABILITY "\","
    ...
    /* sensor: WAN IP */
    snprintf(topic, sizeof(topic),
        "homeassistant/sensor/%s/wan_ip/config", s_device_id);
    snprintf(payload, sizeof(payload),
        "{\"name\":\"WAN IP\","
        "\"uniq_id\":\"%s_wan_ip\","
        "\"stat_t\":\"" TOPIC_STATE "\","
        "\"val_tpl\":\"{{value_json.wan_ip}}\","
        "\"ic\":\"mdi:ip-network\","
        "\"entity_category\":\"diagnostic\","
        "\"avty_t\":\"" TOPIC_AVAILABILITY "\","
        "%s}", s_device_id, dev);
