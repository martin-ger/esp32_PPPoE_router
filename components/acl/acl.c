...
IRAM_ATTR uint8_t acl_check_packet(uint8_t acl_no, struct pbuf *p)
{
    ...
    /* Detect whether this is a raw IP packet or an Ethernet frame.
     * PPP (PPPoE) delivers raw IP — first nibble is the IP version (4 or 6).
     * Ethernet frames start with 6-byte DA + 6-byte SA + 2-byte EtherType. */
    if (payload_len >= 1) {
        uint8_t first_nibble = payload[0] >> 4;
        if (first_nibble == 4) {
            /* Raw IPv4: IHL must be 5-15 */
            ...
            /* Raw IPv6 — not filtered, allow through */
            return ACL_NO_MATCH;
        }
    }
