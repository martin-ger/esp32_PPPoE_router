...
static uint32_t pcap_link_type = 1;  // DLT_EN10MB (Ethernet) by default
...
static void create_pcap_global_header(pcap_global_header_t *hdr)
{
    ...
    hdr->network = pcap_link_type;   // DLT_EN10MB (1) or DLT_RAW (101)
}
...
{
    pcap_link_type = dlt;
    ESP_LOGI(TAG, "PCAP link type set to %lu", (unsigned long)dlt);
}
