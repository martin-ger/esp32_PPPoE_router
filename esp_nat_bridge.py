#!/usr/bin/env python3
"""
ESP32 PPPoE Router MCP Bridge

MCP server that controls an ESP32 PPPoE NAT Router via its remote console (telnet/TCP).
Uses fastmcp and telnetlib3.

The router uses an Ethernet uplink (CONFIG_ETH_UPLINK) with optional PPPoE.
WiFi-only commands (set_sta, scan, set_sta_mac) are not available on this build.
...
mcp = FastMCP(
    "ESP32 PPPoE Router Bridge",
    instructions="Control an ESP32 PPPoE NAT Router (Ethernet uplink) via its remote console (telnet/TCP)",
    lifespan=lifespan,
)
...
async def show_acl() -> str:
    """Show all four firewall ACL lists with hit counts.

    Lists:
      - to_esp:   WAN → Router  (traffic from internet destined for the router itself)
      - from_esp: Router → WAN  (traffic originating from the router going to internet)
      - to_ap:    Client → Internet  (AP client traffic being forwarded upstream)
      - from_ap:  Internet → Client  (upstream traffic being forwarded to AP clients)
    """
    raw = await _cmd("show acl")
    _LABELS = {
        "to_esp":   "=== WAN→Router (to_esp) ===",
        "from_esp": "=== Router→WAN (from_esp) ===",
        "to_ap":    "=== Client→Internet (to_ap) ===",
        "from_ap":  "=== Internet→Client (from_ap) ===",
    }
    sections = re.split(r"(?=^ACL: )", raw, flags=re.MULTILINE)
    parts = []
    for section in sections:
        for key, label in _LABELS.items():
            if section.startswith(f"ACL: {key}"):
                parts.append(label + "\n" + section.replace(f"ACL: {key}", "").strip())
                break
        else:
            if section.strip():
...
# ═══════════════════════════════════════════════════════════════════════════
# WAN / UPLINK CONFIGURATION (Ethernet + PPPoE)
# ═══════════════════════════════════════════════════════════════════════════


@mcp.tool()
async def set_sta_static(ip: str = "", subnet: str = "", gateway: str = "") -> str:
    """Set a static IP for the router's Ethernet WAN interface, or revert to DHCP. Requires restart.

    Only relevant when PPPoE is disabled. When PPPoE is enabled the IP is
    assigned by the ISP during PPPoE negotiation and this setting is ignored.
...
async def set_pppoe(
    ...
    """Configure PPPoE DSL uplink settings. Requires restart to apply.

    Call with no arguments to show current PPPoE configuration.

    Args:
        username: ISP PPPoE username (-u).
        password: ISP PPPoE password (-p).
        service:  PPPoE service name, usually empty (-s).
        auth:     Auth type: 0=auto, 1=PAP, 2=CHAP. -1 to skip (-a).
        enable:   1 to enable PPPoE, 0 to disable (plain Ethernet). -1 to skip (-e).
        vlan:     802.1Q VLAN ID (0=disabled). -1 to skip (-v).
        babyjumbo: 1=baby-jumbo frames/MSS=1460 (default), 0=standard/MSS=1452. -1 to skip (-j).
    """
    if not any([username, password, service, auth >= 0, enable >= 0, vlan >= 0, babyjumbo >= 0]):
        return await _cmd("set_pppoe")
    cmd = "set_pppoe"
    if username:
        cmd += f" -u {username}"
    if password:
...
async def set_hostname(name: str = "") -> str:
    """Set or clear the router's DHCP hostname. Requires restart to apply.

    The hostname is advertised via DHCP and used as the device's network name
    (e.g. visible in ISP CPE device lists). Default is 'esp32-nat-router'.

    Args:
        name: Hostname (max 32 chars, letters/digits/hyphens only).
              Pass empty string to clear and revert to the default.
    """
    return await _cmd(f"set_hostname {name}".strip())


@mcp.tool()
async def set_ap_dns(dns: str = "") -> str:
    """Set or clear the DNS server advertised to AP clients via DHCP. Requires restart.

    By default the router advertises itself as the DNS server for AP clients.
    Set a custom DNS (e.g. "1.1.1.1") to override this.

    Args:
        dns: DNS server IP (e.g. "1.1.1.1", "8.8.8.8") or empty to revert to default.
    """
    return await _cmd(f"set_ap_dns {dns}".strip())
...
async def add_portmap(
    ...
    Args:
        ...
        iface: Interface to bind to - "ETH" (default, Ethernet/PPPoE uplink) or "VPN" (WireGuard tunnel).
    """
    ...
    iface = iface.upper()
    if iface not in ("ETH", "VPN"):
        raise ValueError("Interface must be ETH or VPN")
...
# Four ACL lists exist:
#   to_ap:    Client → Internet  (AP client traffic forwarded upstream)
#   from_ap:  Internet → Client  (upstream traffic forwarded to AP clients)
#   to_esp:   WAN → Router       (traffic from internet to the router itself)
#   from_esp: Router → WAN       (traffic from the router to internet)
_ACL_DIRECTIONS = {
    "upstream":   "to_ap",    # AP client → internet
    "downstream": "from_ap",  # internet → AP client
    "wan_in":     "to_esp",   # WAN → router itself
    "wan_out":    "from_esp", # router itself → WAN
}
...
def _resolve_direction(direction: str) -> str:
    ...
    if key not in _ACL_DIRECTIONS:
        raise ValueError(
            "direction must be 'upstream', 'downstream', 'wan_in', or 'wan_out'"
        )
...
async def acl_add(
    ...
    """Add a firewall ACL rule to the router.

    Use client IPs (e.g. 192.168.4.x) or DHCP device names for src/dst.

    Args:
        direction: One of:
            'upstream'   — AP client → internet (to_ap)
            'downstream' — internet → AP client (from_ap)
            'wan_in'     — WAN → router itself (to_esp)
            'wan_out'    — router itself → WAN (from_esp)
...
async def acl_delete(direction: str, index: int) -> str:
    ...
    Args:
        direction: 'upstream', 'downstream', 'wan_in', or 'wan_out' (see acl_add).
