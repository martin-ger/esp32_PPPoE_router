 ...
 * TTL override, TCP MSS clamping, Path MTU, and PPPoE kill switch.
 *
...
// Original netif input and linkoutput/output function pointers
static netif_input_fn original_netif_input = NULL;
static netif_linkoutput_fn original_netif_linkoutput = NULL;
#if CONFIG_ETH_UPLINK
static netif_output_fn original_netif_output = NULL;  // For PPP netifs (no linkoutput)
#endif
static struct netif *sta_netif = NULL;
#if CONFIG_ETH_UPLINK
static bool sta_is_ppp = false;  // true when hooked netif is PPP (PPPoE mode)
#endif
...
static IRAM_ATTR err_t netif_output_hook(struct netif *netif, struct pbuf *p, const ip4_addr_t *ipaddr) {
    ...
    // TTL override for upstream packets (raw IP — no Ethernet header)
    if (sta_ttl_override > 0 && p != NULL && p->len >= sizeof(struct ip_hdr)) {
    ...
    if (original_netif_output != NULL) {
        return original_netif_output(netif, p, ipaddr);
    }
    return ERR_IF;
}
...
#if CONFIG_ETH_UPLINK
/* __wrap_ip4_input — intercepts all IPv4 input for the PPP netif.
 *
 * ppp_input() calls ip4_input(pb, ppp_netif) directly, bypassing
 * ppp_netif->input, so hooking netif->input would never fire for
 * received packets.  The linker --wrap=ip4_input redirects every call
 * to ip4_input() through here so we can count bytes for the PPP netif
 * before forwarding to the real implementation.
 */
err_t __real_ip4_input(struct pbuf *p, struct netif *inp);
...
IRAM_ATTR err_t __wrap_ip4_input(struct pbuf *p, struct netif *inp) {
    ...
    return __real_ip4_input(p, inp);
}
#endif /* CONFIG_ETH_UPLINK */

#if CONFIG_ETH_UPLINK
/* Clear the PPP-side hook state so init_byte_counter() can reinstall
 * hooks on the next PPPoE session.  Called on disconnect before the
 * old ppp_netif is freed; no need to restore original_netif_output
 * since the netif is about to be destroyed. */
void deinit_byte_counter_ppp(void) {
    original_netif_output = NULL;
    original_netif_input = NULL;
    sta_netif = NULL;
    sta_is_ppp = false;
}
#endif
...
#if CONFIG_ETH_UPLINK
    if (pppoe_enabled) {
        /* PPPoE mode: hook the PPP netif output for sent bytes.
         * Received bytes are counted in __wrap_ip4_input above, which
         * intercepts ip4_input() called directly by ppp_input(). */
        struct netif *ppp_nif = pppoe_get_netif();
        if (ppp_nif != NULL && ppp_nif->output != NULL && original_netif_output == NULL) {
            sta_netif = ppp_nif;
            sta_is_ppp = true;
            original_netif_output = sta_netif->output;
            sta_netif->output = netif_output_hook;
            ESP_LOGI(TAG, "Byte counter initialized for PPP interface (output + ip4_input wrap)");
        }
    } else if (ethNetif != NULL && original_netif_input == NULL) {
...
{
#if LWIP_STATS
    struct stats_ip_napt s;
    ip_napt_get_stats(&s);
    out->tcp  = s.nr_active_tcp;
    out->udp  = s.nr_active_udp;
    out->icmp = s.nr_active_icmp;
    out->total = s.nr_active_tcp + s.nr_active_udp + s.nr_active_icmp;
    out->max  = IP_NAPT_MAX;
#else
